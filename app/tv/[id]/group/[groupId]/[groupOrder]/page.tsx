import { getTVDetail, getEpisodeGroupDetail } from '@/lib/services/tv-service';
...
export default async function EpisodeGroupPage({
    params,
}: {
    params: Promise<{ id: string, groupId: string, groupOrder: string }>
}) {
    const { id, groupId, groupOrder } = await params;
    ...
    const groupDetail = await getEpisodeGroupDetail(groupId);
    if (!groupDetail) {
        notFound();
    }

    // groupOrder here maps to the specific sub-group ID from TMDB (e.g. the specific story arc ID)
    const currentGroup = groupDetail.groups.find(g => g.id === groupOrder);

    if (!currentGroup || !currentGroup.episodes || currentGroup.episodes.length === 0) {
        notFound();
    }
        ...
        <main className="min-h-screen bg-black text-white pb-20 pt-24 px-4 sm:px-8 max-w-5xl mx-auto">
            ...
            <header className="mb-12">
                <div className="flex flex-col mb-4">
                    <span className="text-amber-500 font-medium tracking-wider text-sm uppercase mb-2">{groupDetail.name}</span>
                    <h1 className="text-3xl sm:text-4xl font-bold">{currentGroup.name}</h1>
                </div>

                <p className="text-neutral-400 text-lg">
                    {currentGroup.episodes.length} Episodes
                </p>
            </header>
